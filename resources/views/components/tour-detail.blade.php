<div class=" flex bg-regal-blue items-center justify-between p-y-4  px-8 lg:px-20">
            ...
            <div class="flex flex-col lg:flex-row">
                ...
                <div class="flex items-center gap-x-2">
                    ...
                    <h3 class="uppercase text-xs lg:!text-xl font-semibold text-white">
                        {{$tour->getTranslatedAttribute('name', app()->getLocale())}}
                    </h3>
                </div>
            </div>
...
<div class="my-4 w-full  px-8 lg:px-20 lg:flex">
    <div class="lg:w-1/2">
        <div class="tour-detail-swiper">
            @foreach(json_decode($tour->images) as $img)
                <div class="rounded-[20px] overflow-hidden">
                    <img class="w-full rounded-[20px] h-full" src="/storage/{{$img}}" alt="">
                </div>
            @endforeach
        </div>
    ...
    <div class="w-full flex flex-col justify-between px-8">
        <div>
            <h3 class="mb-1 lg:mb-4 text-xl lg:!text-3xl font-bold text-regal-blue">{{$tour->getTranslatedAttribute('name', app()->getLocale())}}</h3>
            ...
            <div class="mb-4 flex items-center">
                ...
                <p class="font-medium text-[#575757]">{{__('Длительность')}}: {{$tour->getTranslatedAttribute('duration', app()->getLocale())}}</p>
            </div>
            <p class="whitespace-pre-line whitespace-pre-line text-xs lg:text-sm text-[#575757] lg:w-3/5">{{$tour->getTranslatedAttribute('text', app()->getLocale())}}</p>
        </div>
        <div class="my-4 flex justify-between items-center text-center lg:!text-left">
{{--            <p class="mb-2 lg:!text-left font-medium text-center">{{__('Цена устанавливается по запросу')}}</p>--}}
            ...
            @if($tour->price)
                <span class="hidden lg:inline text-sm text-right">{{__('Цены начинаются от').' '.$tour->price.'$ '.__('на человека')}}</span>
            @else
                <span class="hidden lg:inline text-sm text-right">{{__('Цена устанавливается по запросу')}}</span>
            @endif
        </div>
    </div>
</div>
