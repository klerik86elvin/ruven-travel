...
<div class="hidden lg:block whatsapp-icon">
    <a href="https://api.whatsapp.com/send?phone=994516819308">
...
<header class="relative">
    ...
    <div class="menu hidden top-0 lg:block absolute w-full">
        <div class="lg:px-28 lg:py-8 flex justify-between w-full">
            ...
            <ul class="font-bold text-sm text-[#013793] flex gap-x-16">
                <li>
                    <a class="uppercase" href="#tours">{{__('ТУРЫ')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#about-us">{{__('О НАС')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#about-us">{{__('КОНТАКТЫ')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#hotels">{{__('ОТЕЛИ')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#transport">{{__('Транспорт')}}</a>
                </li>
                <li>
    ...
    <div class="mobile-header top-0 absolute flex justify-between w-full  p-8 lg:hidden">
        ...
        <div class="absolute hidden mobile-menu p-2 top-0 right-0 w-1/2 h-screen bg-regal-blue/[.9] z-10">
            ...
            <ul class="font-bold flex flex-col mt-4 mr-6 gap-y-4 mx-auto text-sm text-right text-white">
                <li>
                    <a class="uppercase" href="#tours" onclick="clickMenu()">{{__('ТУРЫ')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#about-us" onclick="clickMenu()">{{__('О НАС')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#about-us" onclick="clickMenu()">{{__('КОНТАКТЫ')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#hotels" onclick="clickMenu()">{{__('ОТЕЛИ')}}</a>
                </li>
                <li>
                    <a class="uppercase" href="#transport" onclick="clickMenu()">{{__('Транспорт')}}</a>
                </li>
                <li>
                    <div class="lang-list flex relative drop-shadow-md flex justify-end">
                        <div class="locales flex gap-x-4 absolute pt-2 drop-shadow-xl">
                            @foreach(\Illuminate\Support\Facades\Config::get('app.locales') as $locale)
                                @if($locale != app()->getLocale())
                                    <a class="flex gap-x-2 items-center my-1" href="lang/{{$locale}}">
                                        <img src="{{asset('assets/img/lang/'.$locale.'.svg')}}" alt="" style="width: 35px">
    ...
    <div class="absolute w-full top-[25%] text-center">
        <p class="text-xl  lg:text-3xl lg:tracking-widest font-bold uppercase text-[#0052E0]">{{__('В ЛЮБОЙ УГОЛОК')}}</p>
        <p class="text-3xl lg:text-7xl lg:tracking-widest font-bold uppercase text-[#013793]">{{__('АЗЕРБАЙДЖАНА')}}</p>
    </div>
</header>
<section id="tours" class="">
    <div class="separator h-4 bg-[#FFD500]"></div>
    <div class="bg-[#013793] px-8 lg:px-28 py-4">
    ...
    <div class="tour-list flex flex-col gap-y-8 my-8 px-8 lg:px-28">
        ...
        <div class="tour bg-white overflow-hidden lg:flex rounded-t-xl lg:rounded-l-xl" style="box-shadow: 0px 14px 23px 19px rgba(0, 0, 0, 0.04);">
            <div class="">
                <img class="lg:h-full lg:w-[620px] object-cover" src="\storage\{{$d->main_image}}" alt="">
            </div>
            <div class="lg:w-1/2 p-4 relative">
                <p data-tour-id="{{$d->id}}" data-modal-target="defaultModal" data-modal-toggle="defaultModal" class="tour-click mb-4 font-bold text-3xl lg:text-5xl text-center lg:text-left text-[#00C8DA] cursor-pointer" type="button">{{$d->getTranslatedAttribute('name', app()->getLocale())}}</p>
                ...
                <div class="mb-4 flex items-center">
                    ...
                    <p class="font-medium text-[#575757]">{{__('Длительность')}}: {{$d->getTranslatedAttribute('duration', app()->getLocale())}}</p>
                </div>
                <p class="whitespace-pre-line lg:text-xl text-[#575757]">{{\Illuminate\Support\Str::limit($d->getTranslatedAttribute('text', app()->getLocale()), 200)}}</p>
                <div class="mt-8 lg:mt-14 flex flex-col items-center lg:!items-start gap-y-2">
                    ...
                    <div class="flex justify-center items-center gap-x-4 w-64 bg-regal-blue text-white rounded-full" style="background: #145BE7;
                        ...
                        @else
                            <span class="mrl-8 text-2xl font-medium"> {{__('По запросу')}}</span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="transport-slider">
            @foreach($transports as $t)
                <div class="justify-end items-end {{$loop->even ? 'lg:!items-start': 'lg:!items-end justify-end'}}">
                    ...
                    <p class="text-xs lg:text-sm mt-10 ml-4 justify-end items-end {{$loop->even ? 'lg:mt-8': 'lg:mb-8 lg:mr-8'}} px-1 py-0.5 rounded-lg bg-[#FFD500] w-max lg:tex-sm">{{$t->getTranslatedAttribute('name', app()->GetLocale())}}</p>
                </div>
            @endforeach
        </div>
    </div>
...
<section id="hotels">
    ...
    <div class=" px-8 py-4 lg:px-28">
        <div class="ext-gray-500 my-4 lg:text-xl text-center">
            <p class="text-gray-500 lg:text-xl text-center">{{__('Нашим клиентам предоставляются лучшие места в отелях с разными вариантами и типами размещения. Для вас все выгоды клиента крупного туроператора:адекватные цены, заранее забронированные билеты и номера в отелях, высокий уровень сервиса.')}}</p>
        </div>
        <div class="hotel-slider">
            @foreach($hotels as $h)
                <div class="">
                    <p class="city-name absolute uppercase font-medium text-xs lg:text-sm justify-end items-end px-2 py-1 mr-0 rounded-none bg-[#FFD500] w-max lg:tex-sm">{{$h->city}}</p>
                    <img src="/storage/{{$h->image}}" alt="">
                    <p class="uppercase text-sm lg:text-md font-bold ">{{$h->getTranslatedAttribute('name', app()->getLocale())}}</p>
                </div>
            @endforeach
        </div>
    </div>
...
<section id="about-us">
    ...
    <div class=" px-8 py-4 lg:px-28">
        <div class="lg:flex">
            <div class=" lg:w-1/2">
                <p class="lg:whitespace-pre-line mb-4 text-regal-blue text-3xl lg:text-5xl font-bold">{{__('Компания Рувен Трэвел
                            приветствует вас!')}}</p>
        ...
        <div class=" flex flex-col-reverse lg:flex-row gap-y-4 w-full lg:justify-between lg:gap-x-36">
            <div class="socials flex flex-col justify-center lg:w-1/2 text-center">
                ...
                <div class="flex justify-center gap-x-4">
                    ...
                    <a target="_blank" href="https://t.me/+994516819308">
                        <img src="{{asset('assets/img/socials/telegram.svg')}}" alt="" style="width: 25px">
                    </a>
                    ...
                    <a target="_blank" href="https://api.whatsapp.com/send?phone=994516819308">
            ...
            <div class="text-white lg:1/2 text-center lg:text-xl flex flex-col justify-center">
                ...
                <div class="flex">
                    <p>{{__('Tel')}}:</p><span>+994 51 681 93 08</span>
                </div>
...
<div id="defaultModal" tabindex="-1" aria-hidden="true" class="bg-regal-blue/[.7] fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full">
    <div class="relative flex justify-center">
        <div class="relative w-full lg:w-11/12 bg-white rounded-lg shadow dark:bg-gray-700">
...
<div id="callToMe" class="!hidden">
    <div class="content lg:mx-auto bg-regal-blue p-4  px-8 lg:px-28 lg:w-3/5">
        ...
        <form class="call-to-me flex flex-col gap-y-4" action="/form/callToMe" method="POST">
            ...
            <div>
                <input class="py-4 text-gray-500 px-2 w-full outline-0 rounded-lg" type="text" name="phone" placeholder="+994 (---) --- -- --" value="994">
            </div>
            ...
            <div class="text-white  text-xl lg:text-3xl">
                <div class="flex mb-4 flex-col lg:flex-row lg:gap-x-6">
                    <div class="lg:w-1/2">
                        ...
                        <div class="custom-select">
                            <p class=" current-value rounded-lg p-2 text-gray-400 bg-white cursor-pointer" onclick="openCustomSelectMenu(this)">{{__('Количество туров')}} <span>0</span></p>
                            <div class="custom-options">
                                @foreach($data as $d)
                                    <p data-id="{{$d->id}}" class="option text-xl relative" onclick="selectTour(this)">{{$d->getTranslatedAttribute('name', app()->getLocale())}}</p>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    <div class="lg:w-1/2 flex gap-x-4">
                        <div>
                            ...
                            <div class="w-32 custom-select">
                                <input type="number" name="adultsCount" class="text-black rounded-lg p-2 w-full outline-0"></input>
                            </div>
                        </div>
                        <div>
                            <p class="mb-2">{{__('Дети ( до 6)')}}</p>
                            <div class="w-32 custom-select" onclick="">
                                <input type="number" name="childCount" class="text-black rounded-lg p-2 w-full outline-0">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="custom-select">
                            ...
                            <div class="custom-options">
                                @foreach($transports as $d)
                                    <p data-id="{{$d->id}}" class="option text-xl relative" onclick="selectTransport(this)">{{$d->getTranslatedAttribute('name', app()->getLocale())}}</p>
                                @endforeach
                            </div>
                            <span class="error-message error-message inline-flex text-sm text-red-700"></span>
                        </div>
                    </div>
                </div>
                <div class="flex gap-x-4 lg:gap-x-6">
                    <div class="lg:w-1/2">
                        ...
                        <input class="outline-0 w-full p-2 rounded-lg text-black" type="text" name="name">
                    </div>
                    <div class="lg:w-1/2">
                        <p class="mb-2 text-xl lg:text-3xl text-white">{{__('Фамилия')}}</p>
                        <input class="outline-0 w-full p-2 rounded-lg text-black" type="text" name="surname">
                    </div>
                </div>
                <div class="flex flex-col lg:flex-row lg:gap-x-6">
                    <div class="w-3/5 lg:w-1/2">
                        <p class="mb-2 text-xl lg:text-3xl text-white">{{__('E-mail')}}</p>
                        <input class="outline-0 w-full p-2 rounded-lg text-black" type="email" name="email">
                    </div>
                ...
                <div>
                    ...
                    <textarea class="w-full rounded-lg p-2 text-black" name="note" id="" cols="30" rows="6"></textarea>
                </div>
            </div>
            <input  type="submit" value="{{__('ПОДТВЕРДИТЬ')}}" class="cursor-pointer uppercase w-max px-4 py-2 font-bold bg-[#FFD500] rounded-lg">
            <div class="hidden loading" role="status">
...
<script>
    ...
    $('input[name= "phone"]').on('click', (e) => {
        $(e.target).css('color', 'black')
    })
    $('input[name= "phone"]').on('focusout', (e) => {
        ...
        $.ajax({
            ...
            error: (e) => {
                ...
                for (const [key, value] of Object.entries(errors)) {
                    $(`input[name="${key}"]`).addClass('class="w-full px-4 py-2 text-red-900 placeholder-red-700 border border-red-500 rounded outline-none focus:ring-red-500 focus:border-red-500 focus:ring-1');
                    if(key === "transport"){
                        $(`input[name="${key}"]`).closest('.custom-select').find('.error-message').html(value);
                    }
                    else {
                        $(`input[name="${key}"]`).after(`<span class="error-message inline-flex text-sm text-red-700">${value}</span>`);
                    }
    ...
    function  selectTour(elem){
        ...
        $(elem).closest('.custom-options').find('.selected').each((key,value) => {
            $(elem).closest('.custom-select').prepend(`<input type="hidden" name=tours[]" value="${$(value).html()}">`)
        })
        $(elem).closest('.custom-select').find('.current-value span').html($(elem).closest('.custom-options').find('.selected').length)
        if ($(elem).closest('.custom-options').find('.selected').length > 0){
            $(elem).closest('.custom-select').find('.current-value').css('color','black');
        }
        else {
            $(elem).closest('.custom-select').find('.current-value').removeAttr('style')
        }
    }
    ...
    async function selectTransport(elem){
        ...
        await $(elem).closest('.custom-options').find('p').removeClass('selected');
        $(elem).closest('.custom-select').removeClass('open');
        $(elem).closest('.custom-select').find('.current-value').css('color','black').html(value);
        $(elem).closest('.custom-select').find('input[type="hidden"]').val($(elem).html());
        $(elem).toggleClass('selected');
    }
</script>
</html>
